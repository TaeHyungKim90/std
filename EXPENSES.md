...
- 기본 설정은 `OCR_PROVIDER=paddleocr`입니다. [PaddleOCR 실행 안내](ocr-service/README.md)에 따라 내부 OCR 서비스를 실행하고 URL·토큰을 설정합니다. 설정 누락·분석 실패는 FAILED로 저장하며 수동 작성을 허용합니다. 예시 데이터가 필요한 개발 환경에서만 `OCR_PROVIDER=mock`을 명시합니다.
- HR은 JPG/PNG 확장자·MIME·파일 시그니처와 10MB 제한을 검사합니다. PaddleOCR 서비스는 추가로 이미지 전체 디코딩, 1,200만 픽셀과 단일 프레임 제한을 검사합니다. 잘못된 이미지의 OCR은 실패 처리되지만 HR에는 원본 첨부가 남습니다.
- 영수증·PENDING 결과를 먼저 커밋하여 분석 대기 중 HR DB의 쓰기 잠금을 유지하지 않습니다. 이후 SUCCEEDED/FAILED를 기록합니다. 프로세스가 중단돼 PENDING이 남으면 수동 작성 또는 재업로드할 수 있습니다.
