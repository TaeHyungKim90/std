...
def read_today_attendance(
    ...
    return service.get_today_attendance(db, user_id, datetime.now().date())

@router.post("/clock-in", response_model=attendance_schemas.AttendanceResponse)
def clock_in(
    req: attendance_schemas.AttendanceRequest, 
    db: Session = Depends(get_db), 
    current_user: dict = Depends(get_current_user)
):
    ...
    return service.create_clock_in(
        db, user_id, now, status="NORMAL", 
        location=req.location_name, lat=req.latitude, lng=req.longitude, note=req.note
    )
...
def clock_out(
    ...
    return service.update_clock_out(db, record.id, now, note=req.note)