...
router = APIRouter(prefix="/hr/todos", tags=["HR Todos"])
...
def read_todo_configs(
    ...
    return service.get_user_config(db, current_user.get("userId"))
...
def update_todo_config(
    ...
    return service.upsert_todo_config(db, current_user.get("userId"), config_in)
...
def read_todos(
    ...
    """내 모든 일정을 조회합니다."""
    return service.get_todos(db, user_id=current_user.get("userId"), skip=skip, limit=limit)
...
def create_todo(
    ...
    """새로운 일정을 생성합니다."""
    return service.create_todo(db=db, todo=todo, user_id=current_user.get("userId"))
